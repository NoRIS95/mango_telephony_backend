...
class MangoAPI:
    ...
    def sms(self, number: str, text: str, from_ext: str, command_id="base", sender=None):
        if number is None:
    ...
    def hangup(self, command_id="base", call_id=None):
        if call_id is None:
            ...
            return self.request({'command_id': command_id, 'call_id': call_id}, 'vpbx/commands/call/hangup')
    ...
    def balance(self):
        return self.request({}, 'vpbx/account/balance')

    def lines(self):
        return self.request({}, 'vpbx/incominglines')

    def audio(self):
        return self.request({}, 'vpbx/audiofiles')
    ...
    def roles(self):
        return self.request({}, 'vpbx/roles')

    def sips_list(self):
        return self.request({}, 'vpbx/sips')
    ...
    def domains_list(self):
        return self.request({}, 'vpbx/domains')

    def trunk_num_list(self):
        return self.request({}, 'vpbx/trunks/numbers')

    def bwlist_state(self):
        return self.request({}, 'vpbx/bwlists/state/')

    def bwlist_nums(self):
        return self.request({}, 'vpbx/bwlists/numbers/')

    def bwlist_add(self, number=None, list_type='white', num_type='tel', comment='API'):
        if number is not None:
            return self.request({'number': number, 'list_type': list_type, 'number_type': num_type, 'comment': comment},
                                'vpbx/bwlists/number/add/')
        else:
            raise ValueError('Please specify params')

    def bwlist_del(self, num_id=None):
        if num_id is not None:
            return self.request({'number_id': num_id}, 'vpbx/bwlists/number/delete/')
        else:
            raise ValueError('Please specify params')

    def campaign_info(self, campaign_id=None):
        if campaign_id is not None:
            return self.request({'campaign_id': campaign_id}, 'vpbx/campaign')
        else:
            raise ValueError('Please specify params')
